...
contract Dac {
	...
	function activateVoting(uint256 _index) public {
		if (!halfToVote) {
			...
			if (teammates.length % 2 == 0) {
				require(
					votings[_index].signers.length > (teammates.length / 2)
				);
			} else {
				require(
					votings[_index].signers.length >=
						((teammates.length + 1) / 2)
				);
			}
		}
	...
	function freezeTeammatesListFrozen()
		public
		contractOnly
		returns (bool success)
	{
	...
	function changePurchasePublic(bool _purchasePublic)
		public
		contractOnly
		returns (bool success)
	{
	...
	function freezePurchaseRuleFrozen()
		public
		contractOnly
		returns (bool success)
	{
	...
	function changeHalfToVote(bool _halfToVote)
		public
		contractOnly
		returns (bool success)
	{
	...
	function freezeHalfToVoteFrozen()
		public
		contractOnly
		returns (bool success)
	{
